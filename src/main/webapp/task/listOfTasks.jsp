			...
			<div class="header flex items-center justify-between">
				...
				<div class="buttons-container">
					...
					<button class="btn btn-danger"
						id="delete-project-<c:out value="${project.projectID}"/>"
						onclick="confirmation(<c:out value="${project.projectID}"/>)">Delete</button>

					<button id="update-task-cta"
						class="button regular-button green-background"
						onclick="window.location.href='ProjectController?action=updateProject&projectID=${project.projectID}';">Update
						Project</button>
					<button id="add-task-cta"
						class="button regular-button blue-background"
						onclick="window.location.href='/taskMate/task/addTask.jsp';">
						Add task</button>

					<button class="sign-out-cta"
						class="button regular-button red-background"
						onclick="window.location.href='login.jsp';">Log out</button>
				</div>
			</div>
		</div>
			...
			<div class="max-width-container flex">
				...
				<div class="radio-container">
					<input type="radio" id="board" name="view-option" value="board"
						class="radio-input" checked onclick="" /> <label for="board"
						class="radio-label"> <iconify-icon
							icon="ic:round-grid-view" style="color: black" width="24"
							height="24"></iconify-icon> <span>Tasks List</span>
					</label>
				</div>
			</div>
		</div>
							...
							<c:if test="${task.taskStatus == 'To Do'}">
								<li class="task-item">
									<button class="task-button">
										...
										<!-- delete icon -->
										<a
											href="/taskMate/TaskController?action=deleteTask&taskID=<c:out value='${task.taskID}'/>">
											<iconify-icon icon="icomoon-free:bin" width="16" height="16"
												style="cursor: pointer;"
												onclick="confirmation('${task.taskID}')"></iconify-icon>
											<!-- arrow --> <iconify-icon
												icon="material-symbols:arrow-back-ios-rounded"
												style="color: black" width="18" height="18"
		...
		<div class="overlay-content green-background">
			...
			<div class="control-buttons-container">
				...
				<button
					...
					<iconify-icon icon="material-symbols:edit-rounded"
