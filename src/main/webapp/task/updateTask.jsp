...
<body>
	<div class="form-frame">
		...
		<h1>Update Task</h1>
		<form class="form" autocomplete="off" action="TaskController"
			method="post">
			<c:if test="${not empty task}">
				<label for="project-name" class="label">Task Name</label>
				<input type="text" name="taskName" id="taskName"
					class="input white-background" required value="${task.taskName}" />

				<label for="task-description" class="label">Description</label>
				<textarea name="description" id="description" rows="5"
					class="textarea-input white-background">${task.description}</textarea>

				<h2 class="label">Start Date</h2>
				<div>
					<input type="date" name="startDate" id="startDate"
						class="input white-background" required value="${task.startDate}" />
				</div>

				<h2 class="label">Due Date</h2>
				<div>
					<input type="date" name="endDate" id="endDate"
						class="input white-background" required value="${task.endDate}" />
				</div>
				...
				<ul id="taskStatus" class="status-dropdown white-background"
					style="display: none;">
				...
				<input type="hidden" name="taskStatus" id="hidden-status"
					value="To Do" />
